...
def save_image(img_tensor, filename, nrow=8):
    """
    Save a batch of images in a grid to disk.
    img_tensor: torch.Tensor of shape (B, C, H, W) or (B, C, W, H)
    """
    if isinstance(img_tensor, torch.Tensor):
        img_tensor = img_tensor.detach().cpu()

    if img_tensor.dim() != 4:
        raise ValueError('save_image expects a 4D tensor')
    ...
    # Handle possible swapped H/W if width and height are not the same
    B, C, H, W = img.shape
    if C == 1:
    ...
    for idx in range(B):
        row = idx // ncol
        col = idx % ncol
        grid[row * H:(row + 1) * H, col * W:(col + 1) * W] = img[idx]
    ...
    def test_epoch(
        ...
        for batch_idx in range(10):
            ...
            if batch_idx == 0 and save_reconstruction:
                ...
                print(f'logger.get_snapshot_dir() {logger.get_snapshot_dir()}')
                
                save_dir = osp.join('./vae_test_outputs',
                                    'r%d.png' % epoch)
