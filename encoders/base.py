...
class BaseEncoder(ABC, nn.Module):
    ...
    def encode(self, img: np.ndarray) -> torch.Tensor:
        ...
        Args:
            img: numpy array of shape (B, H, W, 3) or (H, W, 3) uint8
    ...
    def obs_to_image_tensor(self, img: np.ndarray) -> torch.Tensor:
        """
        Utility: convert a numpy image to a normalised float tensor of shape (B, 3, H, W) in [0, 1].

        Handles both single images (H, W, 3) and batched images (B, H, W, 3).
        """
        if img.ndim == 3:
